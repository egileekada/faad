    ...
    return (
        <div className='w-full h-full flex px-8 py-8 overflow-y-auto bg-[#F9FAFC] border-t border-l border-[#DDE2E5]' > 
            <div style={{width: '70%'}}  className=' relative flex-1 bg-white rounded-2xl' >
                <p className='font-Inter-SemiBold text-xl pt-8 px-8 mb-4' >Notes</p>  
                <ScrollToBottom className=' h-47vh w-auto bg-[#f4f4f4] pl-4 mx-8 rounded-2xl overflow-y-auto pr-6  flex-1 py-4'>
                    {!isLoading && (
                        <>
                            {data.data.user.notes.length !== 0 && (
                                <>
                                    {[...data.data.user.notes].map((item: any)=> {
                                        return(
