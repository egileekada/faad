...
export default function SearchForTrucks(props: any) {
    ...
    return (
        <div className='w-full  ' > 
            {!isLoading && ( 
                <> 
                    <div className='flex font-Inter-SemiBold mt-4 items-center' >  
                            ...
                            <div className='w-full' >
                                <Select onChange={(e)=> OnChangeHandle(e.target.value)} placeholder='Search' fontSize='sm' >
                                    {data.data.trucks.map((item: any, index: any ) => {
                                        return(
                                            <option className='' key={index} value={item._id} >Truck{' '+(index+1)+'  Capacity: '}<span className='font-Inter-Regular' >{item.capacity+'ℓ'}</span></option>
                                        )
                                    })}
                                </Select>
                            </div>  
