...
export default function DealHistory(props: any) {
    ...
    const ClickHandler =(item: any)=> {
        navigate('/dashboard/clientele/dealhistoryinfo')
            ...
            <Table variant='unstyled' >
                ...
                <Tbody >
                    {data.data.delivery.map((item: any, index: any)=> {
                        if(item.deal.companyName === props.name) {
                            return(
                                <Tr onClick={()=> ClickHandler(item._id)}  className=' font-Inter-Regular text-sm cursor-pointer ' key={index} paddingBottom='30px' >
                                    {/* <Td>{index+1}</Td>  */}
                                    <Td>{item.deal.fuelType}</Td> 
                                    <Td>{item.deal.quantity.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")} </Td> 
                                    <Td>{item.deal.askingPrice.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</Td>  
                                    <Td>
                                        <div className='flex items-center' >
                                            {item.deal.email.slice(0,8)+'...'}
                                    ...
                                    <Td>{item.deal._id.slice(0,6)+'...'}</Td>  
                                    <Td>{DateFormat(item.deal.updatedAt)}</Td>  
                                </Tr> 
                            ) 
                        }
                    })}
